...
def search_entity(request):
	...
	if(request.GET):
		...
		paperRelation = db.getPaperEntityRelationbyEntity(entity)
		cooRelation = db.getCooEntityRelationbyEntity(entity)
		if len(entityRelation) == 0:
			#若数据库中无法找到该实体，则返回数据库中无该实体
			ctx= {'title' : '<h2>数据库中暂未添加该专家</h1>'}
			return render(request,'ent.html',{'ctx':json.dumps(ctx,ensure_ascii=False),'user_text':entity})
		else:
			#返回查询结果
			#将查询结果按照"关系出现次数"的统计结果进行排序
			entityRelation = sortDict(entityRelation)

			return render(request,'entity.html',{'entityRelation':json.dumps(entityRelation,ensure_ascii=False),'user_text':entity,'paperRelation':json.dumps(paperRelation,ensure_ascii=False),'cooRelation':json.dumps(cooRelation,ensure_ascii=False)})
	#需要进行类型转换
	return render(request,"ent.html",{'ctx':ctx})
	...
	if(request.GET):
		...
		if(len(entity1) != 0 and len(relation) == 0 and len(entity2) == 0):
			...
			if(len(searchResult)>0):
				result['searchResult'] = json.dumps(searchResult,ensure_ascii=False)
				return render(request,'relation.html',result)
